...
const stateVars = `
  ...
  // Sample connections based on the first few items
...
// Fix the map icon overlap issue.
code = code.replace(/<Map className="w-5 h-5 stroke-\[1.5\]" \/>/g, '<MapIcon className="w-5 h-5 stroke-[1.5]" />');
